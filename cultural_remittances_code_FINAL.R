...
AER_package <- "/Users/alexander.devera25/Downloads/AER"
AER_files <- list.files(AER_package, pattern = "//.R$", recursive = TRUE, full.names = TRUE)
for(file in AER_files) {
  source(file, echo = TRUE)
}

CAR_package <- "/Users/alexander.devera25/Downloads/car"
CAR_files <- list.files(CAR_package, pattern = "//.R$", recursive = TRUE, full.names = TRUE)
...
cultural_panel <- pdata.frame(cultural_remittances_updated_sheet, index = c("Country.Name", "Years"))

#there must be some kin
...
#make sure to replace graduate_students and graduate_students with suitable replacements - different variables necessary 
